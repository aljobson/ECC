...
| F-002 | HIGH | P1 | Duress PIN decoy E2E fails: after Emergency-PIN unlock the assertion `getByText('HIDDEN WALLET', exact)` (spec line 76, 10s timeout) is not satisfied. **TRIAGED 2026-07-11 (read-only):** NOT a stale selector — `HIDDEN WALLET` exists verbatim at `src/pages/DuressPin.jsx:581`, exact-matching the test, and the sibling `REAL WALLET` assertion (line 67) passes. The decoy is a single shared result panel gated on `isDecoy` (not a route change); `isDecoy` is set at `WalletProvider.jsx:1543-1545` on duress-password match. Root cause is most likely **(C) test-infra/timing** on the *second* in-session unlock — candidates: (i) equalizer + up to 3 extra Argon2id KDFs (`WalletProvider.jsx:1486-1531`) pushing latency past the 10s timeout in CI; (ii) `demoUnlock` swallows all errors (`DuressPin.jsx:249-261`) so any throw on the 2nd unlock shows the error banner instead of `HIDDEN WALLET`, looking identical to a routing bug. **(B) real regression cannot be ruled out without a live trace** (triage was static-only). | `e2e/duress-decoy-routing.spec.js:76`; `src/pages/DuressPin.jsx:581,249-261`; `src/lib/WalletProvider.jsx:1428-1560` | NO — needs a headed run with `--trace on` (or a screenshot before line 76) to distinguish timeout vs. error-banner vs. `isDecoy`-never-flips before any code change. Do NOT change the selector — `HIDDEN WALLET` is correct. |
